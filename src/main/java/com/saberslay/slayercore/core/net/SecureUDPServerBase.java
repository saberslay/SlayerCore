...
public abstract class SecureUDPServerBase {
    ...
    public SecureUDPServerBase(int listenPort, int timeoutMs, byte[] masterKey) throws Exception {
    ...
    private void listenLoop() {
        while (running) {
            try {
                ...
                onPacketDecrypted(decrypted, senderAddress, senderPort);
    ...
    public void sendSecure(byte[] plaintext, InetAddress address, int port) {
        try {
            byte[] encrypted = cipher.encrypt(plaintext);

            DatagramPacket packet =
                    new DatagramPacket(encrypted, encrypted.length, address, port);
    ...
    protected abstract void onPacketDecrypted(byte[] data, InetAddress address, int port);
